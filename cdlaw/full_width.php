...
if(get_post_meta($id, "qode_title-image", true) != ""){
 $title_image = get_post_meta($id, "qode_title-image", true) ? get_post_meta($id, "qode_title-image", true) : '/wp-content/uploads/2024/11/blog-default-img-lg.jpg';
}else{
	$title_image = $qode_options_passage['title_image'] ? $qode_options_passage['title_image'] : '/wp-content/uploads/2024/11/blog-default-img-lg.jpg';
}
	...
	<?php get_header(); ?>
		<?php if(!get_post_meta($id, "qode_show-page-title", true)) { ?>
			<div class="title animate <?php if($responsive_title_image == 'no' && $title_image != "" && $fixed_title_image == "yes"){ echo 'has_fixed_background '; } if($responsive_title_image == 'no' && $title_image != "" && $fixed_title_image == "no"){ echo 'has_background'; } if($responsive_title_image == 'yes'){ echo 'with_image'; } ?>" <?php if($responsive_title_image == 'no' && $title_image != ""){ echo 'style="background-image:url('.checkWebpCompatibility($title_image).'); height:'.$title_height.'px;"'; }?>>
				...
				<!-- phone widget -->
				<div class="title_phone"><?php if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar('Title-Phone-Number') ) : ?><?php endif; ?></div>
...
<?php if ( is_page(21) ) : ?> <!-- Add case results -->
<div class="title_case_results"><div class="container_inner"><?php if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar('Header-Case-Results') ) : ?><?php endif; ?></div></div>
<?php endif; ?>
		...
		<?php if(($sidebar == "default")||($sidebar == "")) : ?>
			<?php if (have_posts()) : 
					...
					<?php if ( !is_page ( array (16,19,21) ) ) : ?>							
					<?php if ( function_exists('yoast_breadcrumb') ) {yoast_breadcrumb('<p id="breadcrumbs">','</p>');} ?>                    
					<h1 class="diamond_red"><?php the_title(); ?></h1>
					<?php endif; ?>	
